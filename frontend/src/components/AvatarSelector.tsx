...
import type { AvatarSelectorProps } from "../types/avatar";


export default function AvatarSelector({selected,setSelected}:AvatarSelectorProps) {
    const avatars = [
    ...
    return (
        <div className="overflow-y-auto max-h-[336px] max-w-[576px]">
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {
                    avatars.map((avatar, index) => (
                        <img key={index} className={`rounded-full w-20 h-20 m-2 object-cover hover:scale-[1.05] hover:shadow-lg border-[3px] ${selected === avatar ? "border-[#3cdfff]" : "border-transparent"} transition`} src={avatar} onClick={() => setSelected(avatar)} />
                    ))
                }
            </div>
        </div>
